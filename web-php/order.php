...
if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
    echo json_encode(['status'=>'error','message'=>'Format email tidak valid']);
    // Typo harusnya json_encode bukan jason_encode
    exit;
}
...
try {
    ...
    $stmt->execute([
        ...
        ':email' => $email,
        ':telepon' => $telepon,
        // ':telepon' => $telelpon, harunya ':telepon' => $telepon,
        ':produk' => $produk,
        ':jumlah' => $jumlah
    ]);
    ...
    // Jangan tunjukan error detail di production
    echo json_encode(['status'=>'error','message'=>'Terjadi kesalahan server. Silahkan coba lagi.']);
}
