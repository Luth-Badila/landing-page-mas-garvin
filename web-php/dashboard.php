...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_id'])) {
  ...
  $stmt = $pdo->prepare($sql);
  $stmt->execute([
    // ':nama'=>$nama, ':email'=>$email, 'telepon'=>$telepon, 'produk'=>$produk, 'jumlah'=>$jumlah, 'id'=>$id
    // Harunya pake tanda :
    ':nama'=>$nama, ':email'=>$email, ':telepon'=>$telepon, ':produk'=>$produk, ':jumlah'=>$jumlah, ':id'=>$id
  ]);
  header('Location: dashboard.php');
            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    <?php else: ?>
                      ...
                      <td><?= $o['created_at'] ?></td>
                      <td>
                        <a href="dashboard.php?edit=<?= $o['id'] ?>" class="btn-smal btn-edit">Edit</a>
                        <!-- <a href="dashboard.php?edit=dst" class="btn-smal btn-delete" onclick="return confirm('Hapus Pesanan Ini ?')">Hapus</a> -->
                         <!-- Harusnya -->
                      <a href="dashboard.php?delete=<?= $o['id'] ?>" class="btn-smal btn-delete" onclick="return confirm('Hapus Pesanan Ini ?')">Hapus</a>
                      </td>
                      <?php endif; ?>
                    </tr>
                    <?php endforeach; ?>
